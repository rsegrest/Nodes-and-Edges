...
describe("Layout Inspector tests", () => {
    ...
    it('should lay out inspector panel correctly', () => {
        ...
        expect(inspector.getPosition().x).toBe(10);
        expect(inspector.getPosition().y).toBe(490);
        expect(inspector.getDimensions().width).toBe(600);
        expect(inspector.getDimensions().height).toBe(300);
    });
    ...
    it('should create rows for input, output, and headings for inspector table without position', () => {
        ...
        const inputParams = nodes[0].getInputParameterList();
        const outputParams = nodes[0].getOutputParameterList();
    ...
    it('should set position for parameter rows with offsets', () => {
        ...
        LayoutInspector.assignPositionsToRows(testRows, inspectorPosition, rowHeight, yTableOffset);
        // send to get position function
    ...
    it('should test getPlugParent', () => {
        ...
        const node0 = nodes[0];
        const node1 = nodes[1];
        const node0plug = nodes[0].getPlugByPosition('E');
        const node1plug = nodes[1].getPlugByPosition('W');
